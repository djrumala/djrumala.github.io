...
💡 <b>Please note</b>: that you need to restart jekyll server with this command every time you modify `_config.yml` and when you do modify this file, please also make sure that you also modify `_config.dev.yml`. `_config.dev.yml` is just a copy of `_config.yml`. You need to create this for development needs.
